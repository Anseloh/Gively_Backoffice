    ...
    <b-overlay :show="pageBusy" variant="transparent">
        ...
        <b-card>
            <b-overlay :show="itemBusy">
                <div class="row">
                    <div class="text-right col-12 pb-2" @click="exportEmails()">
                        <span class="btn btn-success"> <feather-icon icon="MailIcon" /> Export</span>
                    </div>
                </div>
                <b-table
                    ...
                    <template #cell(actions)="data">
                         <div class="text-nowrap" v-if="data.item.userId">
                            ...
                            See campaigns
                             ...
                             <b-button size="sm" class="ml-1" pill variant="outline-dark" :to="`/transactions/backings?payerEmail=${data.item.email}`">
                            See backings
                             </b-button>
                         </div>
                    </template>
    ...
    methods: {
        ...
        processExport(e){
            return {
                ...
                Email: e.email,
                Type: e.userType == 1? 'Creator': 'Backer',
                'Email Verified': e.isEmailVerified? 'Verified': 'Unverified',
            }
        },
        exportEmails(page = 1) {
            // this.pageBusy = true;
            API.exportEmails({...this.searchParams, pageNum: page}).then(res => {
                
                if(res.success) {
                    if(page == 1){
                        this.accounts = res.data.list.map(e=> this.processExport(e))
                    }else{
                        this.accounts.push(...res.data.list.map(e=> this.processExport(e)))
                    }
                    if(res.data.hasMore){
                        this.exportEmails(page + 1)
                    }else{
                        // download to file
                        const csvContent = this.jsonToCsv(this.accounts); // Convert JSON to CSV
                        this.downloadCsv("amre-users.csv", csvContent); // Trigger download
                    }
        ...
        jsonToCsv(jsonArray) {
            ...
            const rows = jsonArray.map(obj => Object.values(obj).map(value => `"${value}"`).join(",")); 
